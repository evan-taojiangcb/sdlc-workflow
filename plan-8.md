...
5. 旧项目接入时缺少 baseline intake，容易把现有架构当成 fresh project 处理
...
5. 用 baseline intake 替代“先看几眼目录就开始改老项目”
...
对 existing project，还需要一个前置模式：

- **Existing Project Intake**
  - 先盘点既有结构、脚本、测试和环境依赖
  - 生成 baseline 文档
  - 后续 design / review 必须引用 baseline
...
### 3.4 Existing Project Mode

如果项目已经存在真实代码和工程结构，v8 不应再把它当 fresh project 处理。existing project mode 的 contract 是：

1. 先 intake，再写需求
2. 先确认既有结构，再决定是否允许结构调整
3. 现有技术架构属于 baseline truth source
4. 任何结构调整都必须在 `design.md` 中被明确批准
...
3. 自动识别是 fresh project 还是 existing project
4. 若为 existing project，先执行 intake，生成 baseline 文档
5. 若存在 `OPENCLAW_TRIGGER_USER`，自动创建或更新 `.env`
6. `.env`、`.env.example`、`.gitignore` 关系必须正确

Existing project intake 至少生成：

- `docs/PROJECT_BASELINE.md`
- `docs/EXISTING_STRUCTURE.md`
- `docs/TEST_BASELINE.md`

在这些文档生成之前，不允许直接进入 requirements / design / tasks。
...
5. 若为 existing project，必须引用 baseline，并说明本轮需求是沿用现有结构还是批准的结构调整
...
4. 每完成一个任务后，必须同步回写 `tasks.md` 的任务勾选状态和验收标准勾选状态
