...
```
⚠️ 前置条件：启动 Dev Server（不可跳过）

Agent 必须在执行 Playwright MCP 验收前，自行启动 dev server：

  1. 检测启动命令
     → 读取项目 package.json 的 scripts 字段
     → 常见命令优先级: dev > start > serve
     → 如果存在 turbo/nx monorepo，检查 apps/web 或 apps/server 的启动命令
     → 无法确定时读取 README.md 或 ARCHITECTURE.md 中的启动说明

  2. 后台启动 dev server
     → 使用 run_command 在后台执行（如 npm run dev）
     → 设置合理的启动等待时间（WaitMsBeforeAsync ≥ 3000ms）

  3. 等待 server 就绪
     → 读取 dev server 的终端输出，确认出现 ready/listening/started 等关键词
     → 提取实际监听的 URL 和端口（从输出中解析，如 http://localhost:5173）
     → 如果 10 秒内未就绪，检查错误输出并修复

  4. 验证可访问性
     → 用 browser_navigate(url) 打开首页，确认不是错误页面
     → 如果首页加载失败，检查 dev server 日志排查原因

  ❌ 禁止行为:
     - 不允许因 "dev server 未运行" 而跳过 Stage 4/5
     - 不允许将 Stage 4/5 标记为 "Pending"
     - 不允许让用户手动启动 dev server
...
run_playwright_mcp_verification() {
  ...
  #
  # 3. 等待就绪
  #    → 用 command_status 检查输出，等待 ready/listening/started 关键词
  #    → 从输出中提取实际 URL 和端口
  #    → 10 秒未就绪则检查错误并修复
  #
  # 4. 验证可访问
